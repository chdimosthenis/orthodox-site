...
- **Deterministic enforcement (Claude-edit path)** — `.claude/settings.json` registers a `PostToolUse` hook at `.claude/hooks/regen-saint-og-card.py` that fires on every `Edit` / `Write` / `MultiEdit` to `src/content/saints/*.md`. The hook script reads the tool event JSON from stdin, derives the slug from the file path, checks the diff for `iconUrl:` / `feastDay:` / `name:` (skips pure body edits), and runs `_make_og_cards.py --slug <slug> --force`. Output goes to stderr as `[regen-og-card] ...` lines, visible in the transcript. The hook cannot observe writes from Python scripts that bypass Claude's tools (`fetch_icon.py`, `daily_seed.py`, `calendar_seed.py`) — for those flows the manual `_make_og_cards.py` sweep stays mandatory. The hook + skill pair guarantees that the per-saint OG composite never drifts behind the saint page itself.
